...
/* fmemopen() is available in ESP-IDF's newlib implementation */
/* Define _GNU_SOURCE to ensure fmemopen is available */
#ifndef _GNU_SOURCE
  #define _GNU_SOURCE
#endif
...
#include "cipcommon.h"
#include "cipmessagerouter.h"  /* For GetCipClass */
...
/* Include embedded EDS file, icon file, and memory file wrapper if available */
#ifdef CIP_FILE_OBJECT
  #include "embedded_eds_file.h"
  #include "embedded_icon_file.h"
  #include "memfile.h"  /* Memory file wrapper for embedded files */
#endif

#define STATIC_FILE_OBJECT_NUMBER_OF_INSTANCES 2  /* EDS (200) and Icon (201) */
...
static CipFileObjectValues *eds_file_instance = &file_object_values[0]; /* EDS file instance (200)*/
static CipFileObjectValues *icon_file_instance = &file_object_values[1]; /* Icon file instance (201)*/

static CipBool dummy_attribute;

static const CipUint kCipFileEDSFileInstanceNumber = 0xC8U; //200 (EtherNet/IP standard for EDS)
static const CipUint kCipFileIconFileInstanceNumber = 0xC9U; //201 (EtherNet/IP standard for Icon)
                                         ...
                                         CipMessageRouterResponse *const message_router_response)
{
  ...
  /* Verify file handle is valid before rewinding */
  if(struct_to_instance->file_handle == NULL) {
    OPENER_TRACE_ERR("File handle is NULL in InitiateUpload for instance %d\n", instance->instance_number);
    message_router_response->general_status = kCipErrorResourceUnavailable;
    message_router_response->size_of_additional_status = 0;
    CipFileReleaseUploadSession(struct_to_instance);
    return kCipFileObjectStateFileLoaded;
...
static CipFileObjectState TransferUploadFromInitiateUpload(
  ...
  /* Verify file handle is valid before reading */
  if(struct_to_instance->file_handle == NULL) {
    OPENER_TRACE_ERR("File handle is NULL in TransferUploadFromInitiateUpload for instance %d\n", instance->instance_number);
    message_router_response->general_status = kCipErrorResourceUnavailable;
    message_router_response->size_of_additional_status = 0;
    CipFileReleaseUploadSession(struct_to_instance);
    return kCipFileObjectStateFileLoaded;
  ...
  if( ferror(struct_to_instance->file_handle) ) {
    OPENER_TRACE_ERR("File read error in TransferUploadFromInitiateUpload: %s\n", strerror(errno));
                                         ...
                                         CipMessageRouterResponse *const message_router_response)
{
  ...
  /* Verify file handle is valid before reading */
  if(struct_to_instance->file_handle == NULL) {
    OPENER_TRACE_ERR("File handle is NULL in TransferUpload for instance %d\n", instance->instance_number);
    message_router_response->general_status = kCipErrorResourceUnavailable;
    message_router_response->size_of_additional_status = 0;
    rewind(struct_to_instance->file_handle);
    CipFileReleaseUploadSession(struct_to_instance);
  ...
  const size_t data_send_length = fread(data_to_send,
                                        ...
                                        struct_to_instance->file_handle);
  if( ferror(struct_to_instance->file_handle) ) {
    OPENER_TRACE_ERR("File read error in TransferUpload: %s\n", strerror(errno));
    message_router_response->general_status = kCipErrorResourceUnavailable;
    message_router_response->size_of_additional_status = 0;
    rewind(struct_to_instance->file_handle);
    CipFileReleaseUploadSession(struct_to_instance);
...
EipStatus CreateFileObject(unsigned int instance_nr,
  ...
  InsertAttribute(instance,
                  ...
                  kGetableSingleAndAll);  /* State should be readable via both GetAttributeSingle and GetAttributeAll */
  InsertAttribute(instance,
                  2,
                  kCipStringI,
                  EncodeCipStringI,
                                   ...
                                   CipMessageRouterResponse *const message_router_response)
{
  ...
  if (message_router_request->request_data_size > 0) {       //check if message contains data
    ...
    // Note: We need to modify the data pointer for decoding, so we cast away const
    CipMessageRouterRequest *mutable_request = (CipMessageRouterRequest *)message_router_request;
    const CipOctet *message_data = mutable_request->data;             //get message data pointer

    CipStringI *new_instance_name = CipCalloc( 1, sizeof(CipStringI) );
    CipStringIDecodeFromMessage(new_instance_name, mutable_request);

    mutable_request->data = (CipOctet *)message_data;             //reset message data pointer
                                    ...
                                    CipMessageRouterResponse *const message_router_response)
{
  ...
  // Note: CipStringIDecodeFromMessage modifies the request, so we cast away const
  CipMessageRouterRequest *mutable_request = (CipMessageRouterRequest *)message_router_request;
  ...
  file_instance->file_encoding_format = GetUsintFromMessage(
    (const CipOctet **)&mutable_request->data);
...
EipStatus CipFilePreDeleteCallback(
  ...
  if (NULL == file_instance->delete_instance_data) {
    ...
    // Note: delete_instance_data expects non-const request, so we cast away const
    CipMessageRouterRequest *mutable_request = (CipMessageRouterRequest *)message_router_request;
    internal_state = file_instance->delete_instance_data(instance, mutable_request, message_router_response, NULL, 0);
  }
  return internal_state;
}
...
void CipFileInitializeClassSettings(CipClass *cip_class) {
  ...
  cip_class->max_instance = kCipFileIconFileInstanceNumber; /* Predefined instances: 200 (EDS) and 201 (Icon) */
}
...
void CipFileSetChecksum(CipFileObjectValues *const eds_file_instance) {
  ...
  /* Rewind to beginning of file before calculating checksum */
  rewind(eds_file_instance->file_handle);
  ...
  /* Rewind again after checksum calculation so file is ready for reading */
  rewind(eds_file_instance->file_handle);
...
EipStatus CipFileCreateEDSAndIconFileInstance() {
  ...
  /* Try to use embedded EDS file first */
  #ifdef CIP_FILE_OBJECT
    /* Check if embedded EDS file header was included */
    #ifdef EMBEDDED_EDS_FILE_H_
      FILE *file_handle = NULL;
      
      /* Try fmemopen first (GNU extension, may not be available) */
      #ifndef _GNU_SOURCE
        #define _GNU_SOURCE
      #endif
      /* Declare fmemopen - it might not be available */
      FILE *fmemopen(void *buf, size_t size, const char *mode);
      
      file_handle = fmemopen((void *)embedded_eds_file_data, 
                             embedded_eds_file_size, "rb");
      
      /* Fallback to custom memory file wrapper if fmemopen not available or failed */
      if(file_handle == NULL) {
        /* Use our custom memory file wrapper */
        file_handle = memfile_open(embedded_eds_file_data, 
                                   embedded_eds_file_size, "rb");
        if(file_handle == NULL) {
          OPENER_TRACE_ERR("Failed to create memory file handle for embedded EDS file (tried fmemopen and memfile_open)\n");
          return kEipStatusError;
        }
        OPENER_TRACE_INFO("Using custom memory file wrapper for embedded EDS file (%zu bytes)\n", embedded_eds_file_size);
      } else {
        OPENER_TRACE_INFO("Using fmemopen for embedded EDS file (%zu bytes)\n", embedded_eds_file_size);
      }
      
      eds_file_instance->file_handle = file_handle;
      
      /* Set file size directly from embedded data (bypass ftell which may not work with memory files) */
      eds_file_instance->file_size = embedded_eds_file_size;
    #else
      /* Fallback to SPIFFS if embedded file not available */
      #ifdef FILE_OBJECT_EDS_FILE_LOCATION
        eds_file_instance->file_handle = fopen(FILE_OBJECT_EDS_FILE_LOCATION, "rb");
        if(NULL == eds_file_instance->file_handle) {
          OPENER_TRACE_WARN("EDS file not found at %s - EDS file instance will not be created. SPIFFS may not be mounted or file may not exist.\n", FILE_OBJECT_EDS_FILE_LOCATION);
          return kEipStatusError;
        }
        /* Use standard method for SPIFFS files */
        CipFileSetFileLength(eds_file_instance);
      #else
        OPENER_TRACE_ERR("No EDS file available - embedded header not found and no SPIFFS path defined\n");
        return kEipStatusError;
      #endif
    #endif
  #else
    OPENER_TRACE_ERR("CIP_FILE_OBJECT not enabled\n");
    return kEipStatusError;
  #endif
  
  eds_file_instance->state = kCipFileObjectStateFileLoaded;
  
  /* Only call CipFileSetFileLength if not already set (for embedded files) */
  #ifdef EMBEDDED_EDS_FILE_H_
    /* File size already set above for embedded files */
  #else
    CipFileSetFileLength(eds_file_instance);
  #endif
  ...
  eds_file_instance->file_encoding_format =
    kCipFileObjectFileEncodingFormatBinary;  /* EDS files are binary (matching Micro850 behavior) */
...
EipStatus CipFileInit() {
  if( NULL == ( file_object_class = CreateCipClass(kCipFileObjectClassCode, 7, /* # class attributes */
                                                   ...
                                                   3, /* # class services (GetAttributeAll, GetAttributeSingle, Create) */
                                                   12, /* # instance attributes */
                                                   12, /* # highest instance attribute number */
                                                   8, /* # instance services (GetAttributeAll, GetAttributeSingle, InitiateUpload, UploadTransfer, InitiateDownload, DownloadTransfer, ClearFile, Delete) */
    ...
    OPENER_TRACE_ERR("File Object: CreateCipClass failed!\n");
    return kEipStatusError;
  }
  ...
  eds_instance->data = eds_file_instance; // data struct pointer for instance

  EipStatus eds_status = CipFileCreateEDSAndIconFileInstance(); /* Sets up the EDS file data */
  if (kEipStatusError == eds_status) {
    OPENER_TRACE_WARN("EDS file instance creation failed - File Object will continue without EDS file support\n");
    /* Don't fail File Object initialization - it can still work without the EDS file */
  }
  ...
  if( kEipStatusError == CreateFileObject(kCipFileIconFileInstanceNumber, icon_file_instance, false) ) {
    ...
    /* Load embedded icon file into instance 201 */
    #ifdef EMBEDDED_ICON_FILE_H_
      FILE *icon_file_handle = NULL;
      ...
      icon_file_handle = fmemopen((void *)embedded_icon_file_data, 
                                 embedded_icon_file_size, "rb");
      
      /* Fallback to custom memory file wrapper if fmemopen not available or failed */
      if(icon_file_handle == NULL) {
        /* Use our custom memory file wrapper */
        icon_file_handle = memfile_open(embedded_icon_file_data, 
                                       embedded_icon_file_size, "rb");
        if(icon_file_handle == NULL) {
          OPENER_TRACE_ERR("Failed to create memory file handle for embedded icon file (tried fmemopen and memfile_open)\n");
          /* Fall through to empty instance */
          icon_file_instance->file_handle = NULL;
          icon_file_instance->state = kCipFileObjectStateFileEmpty;
          icon_file_instance->file_size = 0;
          icon_file_instance->file_checksum = 0;
        } else {
          OPENER_TRACE_INFO("Using custom memory file wrapper for embedded icon file (%zu bytes)\n", embedded_icon_file_size);
          icon_file_instance->file_handle = icon_file_handle;
          icon_file_instance->file_size = embedded_icon_file_size;
          icon_file_instance->state = kCipFileObjectStateFileLoaded;
        }
      } else {
        OPENER_TRACE_INFO("Using fmemopen for embedded icon file (%zu bytes)\n", embedded_icon_file_size);
        icon_file_instance->file_handle = icon_file_handle;
        icon_file_instance->file_size = embedded_icon_file_size;
        icon_file_instance->state = kCipFileObjectStateFileLoaded;
      }
      ...
      if(icon_file_instance->file_handle != NULL && embedded_icon_file_size > 0) {
        ...
        if(icon_file_instance->file_handle != NULL) {
          /* Close the file handle if it was created but file is empty */
          fclose(icon_file_instance->file_handle);
        }
        icon_file_instance->file_handle = NULL;
        icon_file_instance->state = kCipFileObjectStateFileEmpty;
        icon_file_instance->file_size = 0;
        icon_file_instance->file_checksum = 0;
        OPENER_TRACE_INFO("Icon file is empty (no icon in EDS) - instance 201 set to FileEmpty\n");
      }
    #else
      /* No embedded icon file - create empty instance */
      icon_file_instance->file_handle = NULL;
      icon_file_instance->state = kCipFileObjectStateFileEmpty;
      icon_file_instance->file_size = 0;
      icon_file_instance->file_checksum = 0;
    ...
    icon_file_instance->file_format_version = 1;
    icon_file_instance->invocation_method = kCipFileInvocationMethodNotApplicable;
    icon_file_instance->file_save_parameters = 0;
    icon_file_instance->file_access_rule = kCipFileObjectFileAccessRuleReadOnly;
    icon_file_instance->file_encoding_format = kCipFileObjectFileEncodingFormatCompressedFile; /* Rockwell PLCs use compressed icon files */
    
    const char icon_instance_name[] = "Related EDS and Icon Files";
    CipFileSetInstanceName(icon_file_instance, icon_instance_name, sizeof(icon_instance_name));
    
    /* Set icon file name only if icon file is available */
    if(icon_file_instance->file_handle != NULL && icon_file_instance->file_size > 0) {
      /* Set icon file name to "EDSCollection.gz" to match Rockwell PLC convention */
      char icon_file_name_string[] = "EDSCollection.gz";
      icon_file_instance->file_name.number_of_strings = 1;
      icon_file_instance->file_name.array_of_string_i_structs = CipCalloc(
        icon_file_instance->file_name.number_of_strings, sizeof(CipStringIStruct) );
      icon_file_instance->file_name.array_of_string_i_structs[0].language_char_1 = 'e';
      icon_file_instance->file_name.array_of_string_i_structs[0].language_char_2 = 'n';
      icon_file_instance->file_name.array_of_string_i_structs[0].language_char_3 = 'g';
      icon_file_instance->file_name.array_of_string_i_structs[0].character_set =
        kCipStringICharSet_ISO_8859_1_1987;
      icon_file_instance->file_name.array_of_string_i_structs[0].char_string_struct =
        kCipShortString;
      icon_file_instance->file_name.array_of_string_i_structs[0].string = CipCalloc(
        1, sizeof(CipShortString) );
      CipShortString *icon_file_name_short_string =
        (CipShortString *) (icon_file_instance->file_name.array_of_string_i_structs[0].string);
      icon_file_name_short_string->length = sizeof(icon_file_name_string) - 1;
      icon_file_name_short_string->string =
        CipCalloc( sizeof(icon_file_name_string), sizeof(EipByte) );
      memcpy( icon_file_name_short_string->string, icon_file_name_string,
              sizeof(icon_file_name_string) );
    } else {
      /* No icon file - leave file name empty */
      icon_file_instance->file_name.number_of_strings = 0;
      icon_file_instance->file_name.array_of_string_i_structs = NULL;
    }
    
    CipFileSetDownloadAndClearNotSupported(icon_file_instance);
    icon_file_instance->delete_instance_data = NULL; // not deletable
    icon_instance->data = icon_file_instance; // data struct pointer for instance
  }
