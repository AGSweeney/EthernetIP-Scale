...
- Changes to `enabled`, `byte_offset`, and `unit` take effect immediately
...
- `samples`: Integer (optional, default: 10) - Number of samples to average for tare/known-weight calibration (1-50). Not used for AFE calibration.
...
- More samples (averaging) = more accurate but slower calibration
...
7. **NAU7802 Calibration**: 
   - **Software Calibration (Tare/Known Weight):**
     ...
     - Uses averaging (configurable samples, default: 10) for accuracy
...
curl -X POST http://172.16.82.99/api/nau7802/calibrate \
  -H "Content-Type: application/json" \
  -d '{"action": "calibrate", "known_weight": 100.0, "samples": 10}'
...
fetch('/api/nau7802/calibrate', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ action: 'tare', samples: 10 })
})
  .then(r => r.json())
